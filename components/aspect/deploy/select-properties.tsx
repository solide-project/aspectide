...
interface SelectPropertiesProps extends React.HTMLAttributes<HTMLDivElement> {
    setProperties: Function
}

export function SelectProperties({ setProperties }: SelectPropertiesProps) {
                    ...
                    {Object.entries(items).map(([key, val]: [string, string]) => (
                        <div key={key}>
                            <div className="flex items-center space-x-3">
                                <label htmlFor={key}>{`${key}: ${val.length > 12 ? val.slice(0, 12) + "..." : val
                                    }`}</label>
                                <Trash

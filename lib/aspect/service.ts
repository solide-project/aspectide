...
import { AspectTransactionReceipt, ContractAspect, KVPair, RPC } from "./interface"
...
export class AspectSDK {
    ...
    async deploy(
        ...
        let signedTx: ethers.TransactionResponse = await signer.sendTransaction(
            tx
        )
    ...
    async getAspect(contractAddress: string): Promise<ContractAspect[]> {
        ...
        let result: ContractAspect[] = await aspectCore.methods
            .aspectsOf(contractAddress)
            .call()
